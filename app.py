...
def lime_predict_proba(text_list):
    """
    LIME passes a list of raw texts.
    We tokenize + pad + get CNN probs, then return [P(REAL), P(FAKE)].
    """
    seqs = tokenizer.texts_to_sequences(text_list)
    padded = pad_sequences(seqs, maxlen=max_len, padding="post")
    probs = model.predict(padded).flatten()  # prob of FAKE
    # shape (n, 2) = [P(REAL), P(FAKE)]
...
def get_lime_explanation(text, num_features=8):
    ...
    return exp.as_list()  # list of (word, weight)


def render_lime_explanation(text, num_features=8):
    """
    Show LIME explanation as:
    - little colored cards
    - horizontal bars (normalized)
    """
    lime_words = get_lime_explanation(text, num_features=num_features)
    if not lime_words:
        st.write("No explanation available.")
        return

    abs_vals = [abs(w) for _, w in lime_words]
    max_abs = max(abs_vals) if abs_vals else 1.0

    st.write("Top contributing words (LIME):")

    for word, weight in lime_words:
        # Direction: pushes toward FAKE (positive) or REAL (negative)
        pushes_fake = weight > 0
        label = "FAKE" if pushes_fake else "REAL"
        color = "#ef4444" if pushes_fake else "#22c55e"
        emoji = "🟥" if pushes_fake else "🟩"

        width = int((abs(weight) / max_abs) * 100)
        width = max(5, min(width, 100))  # min 5%, max 100%
        ...
        ">
            <div style="font-size:14px; margin-bottom:3px;">
                ...
                <span style="font-size:12px; color:#6b7280;">
                    &nbsp;→ pushes <b>{label}</b> (weight = {weight:.3f})
                </span>
            </div>
            <div style="
                width:100%;
